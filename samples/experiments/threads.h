...
/* Stack Size */
#define APP_THREADS_STACK_SIZE 4096*16 
...
#define APP_REGISTER_SIG_HDLR(plat_ctxt, sig_id, func, args) \
    do { \
        (plat_ctxt)->sig_hdlr[(sig_id)].hdlr_fn = (func); \
        (plat_ctxt)->sig_hdlr[(sig_id)].arg     = (args); \
     } \
     while (0)
