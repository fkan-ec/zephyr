...
class GdbStub_AARCH64(GdbStub):
    ...
    def parse_arch_data_block(self):
        arch_data_blk_struct = "<QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ"
        arch_data_blk = self.logfile.get_arch_data()['data']
        #temp = len(arch_data_blk)
        #logger.error(f"arch_data_blk length {temp}")
        #temp = struct.calcsize(arch_data_blk_struct)
        #logger.error(f"arch_data_blk_struct len {temp}")
        ...
        self.registers[RegNum.X30] = tu[30]
        self.registers[RegNum.SP] = tu[31]
        self.registers[RegNum.PC] = tu[32]
        self.registers[RegNum.CPSR] = tu[33]

        #logger.error(f"{tu[0]} {tu[1]} {tu[2]} {tu[3]} {tu[4]} {tu[5]} {tu[6]} {tu[7]}")
        #logger.error(f"{tu[8]} {tu[9]} {tu[10]} {tu[11]} {tu[12]} {tu[13]} {tu[14]} {tu[15]}")
        #logger.error(f"{tu[16]} {tu[17]} {tu[18]} {tu[19]} {tu[20]} {tu[21]} {tu[22]} {tu[23]}")
        #logger.error(f"{tu[24]} {tu[25]} {tu[26]} {tu[27]} {tu[28]} {tu[29]} {tu[30]} {tu[31]} {tu[32]} {tu[33]}")
    ...
    def handle_register_single_read_packet(self, pkt):
        ...
        # other than the general ones (e.g. X0, X1)
